...
-- Chisel description
description = "internal chisel, creates the json for the wdeepsys summary page."
short_description = "wdeepsys summary generator"
category = "NA"
hidden = true
...
function add_summaries(ts_s, ts_ns, dst, src)
	local time = deepsys.make_ts(ts_s, ts_ns)
...
-------------------------------------------------------------------------------
-- Helpers to dig into the data coming from deepsys
-------------------------------------------------------------------------------
...
function parse_thread_table_startup()
	...
	local ttable = deepsys.get_thread_table(deepsys.get_filter())
...
function parse_thread_table_interval()
	local data = {}
	local cnt = 0
	local ttable = deepsys.get_thread_table_nofds(deepsys.get_filter())
...
function parse_container_table()
	...
	local ctable = deepsys.get_container_table()
...
function on_init()
	...
	if(deepsys.get_filter() ~= nil and deepsys.get_filter() ~= '') then
		disable_index = true
	end
...
function on_capture_start()
	...
	if not disable_index then
		local dirname = deepsys.get_evtsource_name() .. '_wd_index'
...
--print(json.encode(ssummary.connectionCount, { indent = true }))
	...
	if nintervals % percent_update_sample_period == 0 then
		print('{"progress": ' .. deepsys.get_read_progress() .. ' },')
		io.flush(stdout)
	end
...
function build_output(captureDuration)
	...
	local jtable = {
		info={
			...
			startTs = deepsys.get_firstevent_ts(),
			endTs = deepsys.get_lastevent_ts()
		},
		metrics=res}
	local filter = deepsys.get_filter()

	update_table_counts()

	if should_include(gsummary.notifications) then
		res[#res+1] = {
			name = 'Deepsys Secure Notifications',
			desc = 'Deepsys Secure notifications. Deepsys secure inserts a "notification" event in the capture stream each time a policy triggers. This metric counts the notifications. Chart it over time to compare the other metrics with the point in time where policies were triggered.',
			category = 'general',
...
function on_capture_end(ts_s, ts_ns, delta)
	if arg_file_duration == nil then
		deepsys.run_deepsys('-r "' .. deepsys.get_evtsource_name() .. 
			'" -c wdeepsys_summary ' .. arg_n_timeline_samples .. ',' .. delta .. ' ' ..
			deepsys.get_filter())
		return true
	end
