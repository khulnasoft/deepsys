...
cdeepsys - the ncurses user interface for deepsys

SYNOPSIS
--------

**cdeepsys** [*option*]... [*filter*]

DESCRIPTION
-----------

cdeepsys exports deepsys's functionality through an intuitive and powerful ncurses-based user interface.

cdeepsys has been designed to mimic tools like **top** and **htop**, but it offers richer functionality, including:

- Support for both live analysis and deepsys trace files. Trace files can come from the same machine or from another machine. 
...
- Support for deepsys's filtering language.
- Container support by design.

cdeepsys works on any terminal, and has support for colors and mouse input.

**Views**  

cdeepsys is based on the concept of 'views', little Lua scripts that determine how metrics are collected, processed and represented on screen. Including a new visualization to cdeepsys doesn't require to update the program, and is simply a matter of adding a new view. Views rely on the deepsys processing engine, and this means that they can include any deepsys filter field. Views are located in the deepsys chisel directory path, usually */usr/share/deepsys/chisels* and *~/.chisels*.


BASIC USAGE
-----------

Here are some basic tips to get you started with deepsys:

1. If you run cdeepsys without arguments, it will display live system data, updating every 2 seconds. To analyze a trace file, use the -r command line flag.
...
5. You can see deepsys events for the currently selected entity by typing _F6_
...
CONTAINERS SUPPORT
------------------
Starting cdeepsys with the -pc command line switch will cause many of the views to include additional container information. For example, the _Processes_ will include a column showing the container the process belongs to. Similarly, the _Connections_ view will show which container each connection belongs to.
...
**F6, d**  
  'dig' into the selection, i.e. view deepsys events for the currently highlighted entry. Refer to the deepsys man page to learn about interpreting the content of this window.  
...
- Clicking on the filter string at the top of the screen (the text after 'Filter:') lets you change the deepsys filter and customize the view content.
...
**-E**, **--exclude-users**  
  Don't create the user/group tables by querying the OS when deepsys starts. This also means that no user or group info will be written to the tracefile by the -w flag. The user/group tables are necessary to use filter fields like user.name or group.name. However, creating them can increase deepsys's startup time.  
...
**-m** _url[,marathon-url]_, **--mesos-api=**_url[,marathon-url]_
  Enable Mesos support by connecting to the API server specified as argument (e.g. http://admin:password@127.0.0.1:5050). Mesos url is required. Marathon url is optional, defaulting to auto-follow - if Marathon API server is not provided, cdeepsys will attempt to retrieve (and subsequently follow, if it migrates) the location of Marathon API server from the Mesos master. Note that, with auto-follow, cdeepsys will likely receive a cluster internal IP address for Marathon API server, so running cdeepsys with Marathon auto-follow from a node that is not part of Mesos cluster may not work. Additionally, running cdeepsys with Mesos support on a node that has no containers managed by Mesos is of limited use because, although cluster metadata will be collected, there will be no Mesos/Marathon filtering capability. The API servers can also be specified via the environment variable DEEPSYS_MESOS_API.
...
**-pc**, **-pcontainers**_  
  Instruct cdeepsys to use a container-friendly format in its views. This will cause several of the views to contain additional container-related columns.
...
**-v** _view_id_, **--views**=_view_id_  
  Run the view with the given ID when cdeepsys starts. View IDs can be found in the view documentation pages in cdeepsys. Combine this option with a command line filter for complete output customization.
...
FILTERING
---------
Similarly to what you do with deepsys, you can specify a filter on the command line to restrict the events that cdeepsys processes. To modify the filter while the program is running, or to add a filter at runtime, click on the filter text in the UI with the mouse.

CUSTOMIZING CDEEPSYS
-------------------
cdeepsys is completely customizable. This means that you can modify any of the cdeepsys views, and even create your own views. Like deepsys chisels, cdeepsys views are Lua scripts. Full information can be found at the following github wiki page: https://github.com/khulnasoft/deepsys/wiki/cdeepsys-View-Format-Reference.
...
KhulnaSoft, Ltd. (dba Deepsys) <info@deepsys.com>

SEE ALSO
--------

**deepsys**(8), **strace**(8), **tcpdump**(8), **lsof**(8)
