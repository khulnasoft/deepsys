...
{
    printf(
"cdeepsys version " DEEPSYS_VERSION "\n"
"Usage: cdeepsys [options] [filter]\n\n"
"Options:\n"
...
" -pc, -pcontainer\n"
"                    Instruct cdeepsys to use a container-friendly format in its\n"
"                    views.\n"
...
"                    Run the view with the given ID when cdeepsys starts.\n"
"                    View IDs can be found in the view documentation pages in\n"
"                    cdeepsys. Combine  this option with a command line filter for\n"
...
"4. you can observe reads and writes (F5) or see deepsys events (F6) for any\n"
"   selection.\n"
...
string g_version_string = DEEPSYS_VERSION;

deepsys_init_res cdeepsys_init(int argc, char **argv)
{
			...
			if(it.m_viewinfo.m_valid)
			{
				...
				if(output_type != sinsp_table::OT_JSON)
				{
					if(std::find(it.m_viewinfo.m_tags.begin(), 
						...
						"nocdeepsys") != it.m_viewinfo.m_tags.end())
					{
						continue;
					}
				}
