...
{
    printf(
"deepsys version " DEEPSYS_VERSION "\n"
"Usage: deepsys [options] [-p <output_format>] [filter]\n\n"
"Options:\n"
...
"                    ./chisels, ~/.chisels and /usr/share/deepsys/chisels.\n"
#endif
...
" -D, --debug        Capture events about deepsys itself, display internal events\n"
...
"                    deepsys starts. This also means that no user or group info\n"
...
"                    increase deepsys's startup time. Moreover, they contain\n"
...
"                    Option can also be provided via the environment variable DEEPSYS_K8S_API_CERT.\n"
...
"                    captured. Note that deepsys will enable extended /dev/null\n"
...
"                    emitted by deepsys to be flushed, which generates higher CPU\n"
"                    usage but is useful when piping deepsys's output into another\n"
...
"                    -v will also make deepsys print some summary information at\n"
...
"\n"
"Output format:\n\n"
"By default, deepsys prints the information for each captured event on a single\n"
...
static void add_chisel_dirs(sinsp* inspector)
{
	...
	//
	inspector->add_chisel_dir(DEEPSYS_INSTALLATION_DIR CHISELS_INSTALLATION_DIR, false);

	//
	// Add the directories configured in the DEEPSYS_CHISEL_DIR environment variable
	//
	char* s_user_cdirs = getenv("DEEPSYS_CHISEL_DIR");
...
//
deepsys_init_res deepsys_init(int argc, char **argv)
{
	deepsys_init_res res;
	...
	try
	{
                                        ...
                                        "w:xXz", long_options, &long_index)) != -1)
		{
			...
			case 'A':
				if(event_buffer_format != sinsp_evt::PF_NORMAL)
				{
					...
					delete inspector;
					return deepsys_init_res(EXIT_SUCCESS);
				}
			...
			case 't':
				{
					...
					else
					{
						...
						delete inspector;
						return deepsys_init_res(EXIT_FAILURE);
					}
				}
				break;
			case 'T':
		...
		for(uint32_t j = 0; j < infiles.size() || infiles.size() == 0; j++)
		{
			...
			if(k8s_api)
			{
				if(!k8s_api_cert)
				{
					if(char* k8s_cert_env = getenv("DEEPSYS_K8S_API_CERT"))
					{
			...
			else if(char* k8s_api_env = getenv("DEEPSYS_K8S_API"))
			{
				if(k8s_api_env != NULL)
				{
					if(!k8s_api_cert)
			...
			else if(char* mesos_api_env = getenv("DEEPSYS_MESOS_API"))
			{
...
int main(int argc, char **argv)
{
	...
	if(res.m_next_run_args.size() != 0)
	{
		...
		res = deepsys_init(newargc, &(newargv[0]));
	}
#ifdef _WIN32
	_CrtDumpMemoryLeaks();
#endif
