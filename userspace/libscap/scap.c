						   ...
						   bool import_users)
{
	...
	for(j = 0, all_scanned_devs = 0; j < handle->m_ndevs && all_scanned_devs < max_devs; all_scanned_devs++)
	{
		...
		if((handle->m_devs[j].m_fd = open(filename, O_RDWR | O_SYNC)) < 0)
		{
			...
			else if(errno == EBUSY)
			{
				...
				snprintf(error, SCAP_LASTERR_SIZE, "Too many deepsys instances attached to device %s. Current value for /sys/module/" PROBE_DEVICE_NAME "_probe/parameters/max_consumers is '%"PRIu32"'.", filename, curr_max_consumers);
			}
			else
			{
	...
	//
	// Now that deepsys has done all its /proc parsing, start the capture
	//
...
{
	char* p = getenv("DEEPSYS_HOST_ROOT");
	if(!p)
	{
		p = "";
	}
