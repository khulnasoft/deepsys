/*
Copyright (C) 2013-2015 KhulnaSoft, Ltd.
...
extern int32_t g_cdeepsys_screen_w;
...
json_spy_renderer::json_spy_renderer(sinsp* inspector, 
	...
	int32_t viz_type, 
	spy_text_renderer::deepsys_output_type sotype, 
...
#ifndef NOCURSESUI
	spy_text_renderer::deepsys_output_type dig_otype = spy_text_renderer::OT_NORMAL;
...
void sinsp_cursesui::render_filtersearch_main_menu()
{
	...
	if(m_output_filtering)
	{
		...
		if(*str == "" && m_is_filter_deepsys && m_complete_filter != "")
		{
...
void sinsp_cursesui::switch_view(bool is_spy_switch)
{
	...
	if(psv != NULL)
	{
		if(m_sel_hierarchy.size() > 0)
		{
			...
			m_sel_hierarchy.push_back(psinfo->m_field, psinfo->m_val,
				...
				NULL, psv->m_sortingcol, m_manual_filter, m_is_filter_deepsys,
	...
	//
	// Clear the manual filter, but not if this is a deepsys filter and we're in the same
	// view (applying deepsys filters causes the same view to the reloaded, and in that
	...
	bool is_dig)
{
	...
	m_sel_hierarchy.push_back(field, val, column_info, 
		...
		&rowkeybak, srtcol, m_manual_filter, m_is_filter_deepsys, 
...
bool sinsp_cursesui::spectro_selection(string field, string val,
	sinsp_view_column_info* column_info,
	filtercheck_field_info* info, deepsys_table_action ta)
{
	uint32_t j = 0;
	string spectro_name;
...
bool sinsp_cursesui::drillup()
{
	if(m_sel_hierarchy.size() > 0)
	{
		...
		else
		{
			...
			m_is_filter_deepsys = sinfo->m_prev_is_filter_deepsys;
		}
		...
		//
		if(m_manual_filter != "" && !m_is_filter_deepsys)
		{
...
deepsys_table_action sinsp_cursesui::handle_textbox_input(int ch)
{
	bool closing = false;
	string* str = NULL;
	bool handled = true;
	...
	switch(ch)
	{
		case KEY_F(1):
			...
			return STA_NONE;
		case KEY_F(2):
			m_is_filter_deepsys = !m_is_filter_deepsys;
			*str = "";
			m_cursor_pos = 0;
			render();
			return STA_NONE;
		...
		case KEY_F(4):
			...
			if(m_is_filter_deepsys && !m_output_searching)
			{
				if(*str != "")
				{
					...
					catch(sinsp_exception e)
					{
						...
						//
						string wstr = "Invalid deepsys filter";
	...
	if(closing)
	{
		deepsys_table_action res = STA_NONE;
...
deepsys_table_action sinsp_cursesui::handle_input(int ch)
{
	//
	...
	if(m_mainhelp_page != NULL)
	{
		deepsys_table_action actn = m_mainhelp_page->handle_input(ch);
	...
	if(m_view_sidemenu != NULL)
	{
		...
		deepsys_table_action ta = m_view_sidemenu->handle_input(ch);
	...
	else
	{
		if(m_action_sidemenu != NULL)
		{
			deepsys_table_action ta = m_action_sidemenu->handle_input(ch);
			if(ta == STA_SWITCH_VIEW)
			{
				sinsp_view_info* vinfo = get_selected_view();
		...
		if(m_view_sort_sidemenu != NULL)
		{
			deepsys_table_action ta = m_view_sort_sidemenu->handle_input(ch);
	...
	if(m_spy_box != NULL)
	{
		...
		deepsys_table_action actn = m_spy_box->handle_input(ch);
	...
	if(m_viewinfo_page && m_view_sidemenu == NULL)
	{
		...
		deepsys_table_action actn = m_viewinfo_page->handle_input(ch);
	...
	if(m_viz)
	{
		deepsys_table_action actn = m_viz->handle_input(ch);
		if(actn != STA_PARENT_HANDLE)
		{
			return actn;
		}
	...
	{
		deepsys_table_action actn = m_spectro->handle_input(ch);
		if(actn != STA_PARENT_HANDLE)
		{
			return actn;
		}
...
void sinsp_cursesui::run_action(sinsp_view_action_info* action)
{
	...
	{
		printf("Command finished. Press ENTER to return to cdeepsys.");
		fflush(stdout);
...
//
bool sinsp_cursesui::execute_table_action(deepsys_table_action ta, uint32_t rownumber, bool* res)
{
	//
