...
void sinsp_parser::init_metaevt(metaevents_state& evt_state, uint16_t evt_type, uint16_t buf_size)
{
	...
	evt_state.m_metaevt.m_info = &(g_infotables.m_event_info[PPME_DEEPSYSEVENT_X]);
...
void sinsp_parser::process_event(sinsp_evt *evt)
{
	...
	//
	// When debug mode is not enabled, filter out events about deepsys itself
	//
#if defined(HAS_CAPTURE)
	if(is_live && !m_inspector->is_debug_enabled())
	{
		if(evt->get_tid() == m_inspector->m_deepsys_pid &&
			etype != PPME_SCHEDSWITCH_1_E &&
			etype != PPME_SCHEDSWITCH_6_E &&
			etype != PPME_DROP_E &&
			etype != PPME_DROP_X &&
			etype != PPME_DEEPSYSEVENT_E &&
			etype != PPME_PROCINFO_E &&
			m_inspector->m_deepsys_pid)
		{
