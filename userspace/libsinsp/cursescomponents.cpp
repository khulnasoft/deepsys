...
//
deepsys_table_action curses_table_sidemenu::handle_input(int ch)
{
	int32_t prev_select;
	int input;
...
///////////////////////////////////////////////////////////////////////////////
curses_textbox::curses_textbox(sinsp* inspector, sinsp_cursesui* parent, int32_t viz_type, spy_text_renderer::deepsys_output_type sotype)
{
...
// Return true if the parent should handle the event
//
deepsys_table_action curses_textbox::handle_input(int ch)
{
	if(m_sidemenu)
	{
		deepsys_table_action ta = m_sidemenu->handle_input(ch);
...
deepsys_table_action curses_viewinfo_page::handle_input(int ch)
{
	int32_t totlines;
...
curses_mainhelp_page::curses_mainhelp_page(sinsp_cursesui* parent)
{
	...
	m_ctext->printf("cdeepsys %s. See man page for full documentation\n",
	...
	wattrset(m_win, parent->m_colors[sinsp_cursesui::HELP_BOLD]);
	m_ctext->printf("How to use cdeepsys\n",
		g_version_string.c_str());
...
"4. you can observe reads and writes (F5) or see deepsys events (F6) for any selection.\n\n"
);
...
"Starting cdeepsys with the -pc command line switch will cause many of the views to include additional container information. For example, the _Processes_ will include a column showing the container the process belongs to. Similarly, the _Connections_ view will show which container each connection belongs to.\n\n"
	...
	m_ctext->printf("F4 \\");
	wattrset(m_win, parent->m_colors[sinsp_cursesui::PROCESS]);
	m_ctext->printf(": filter(freetext or deepsys)\n");
	...
	m_ctext->printf("   F6 d");
	wattrset(m_win, parent->m_colors[sinsp_cursesui::PROCESS]);
	m_ctext->printf(": deepsys output for selection     ");
	...
	//
	wattrset(m_win, parent->m_colors[sinsp_cursesui::HELP_BOLD]);
	m_ctext->printf("\nKeyboard Shortcuts for the 'Echo' and 'Deepsys' Windows\n",
		g_version_string.c_str());
	...
	m_ctext->printf("Clicking on column headers lets you sort the table.\n"
		...
		"Clicking on the filter string at the top of the screen lets you change the deepsys filter.\n"
	...
	wattrset(m_win, parent->m_colors[sinsp_cursesui::PROCESS]);
	m_ctext->printf("cdeepsys is completely customizable. This means that you can modify any of the cdeepsys views, "
		"and even create your own views. Like deepsys chisels, cdeepsys views are Lua scripts. Full information can "
		"be found at the following github wiki page: https://github.com/khulnasoft/deepsys/wiki/cdeepsys-View-Format-Reference.\n");

	//
	// deepsys cloud
	//
	wattrset(m_win, parent->m_colors[sinsp_cursesui::HELP_BOLD]);
	m_ctext->printf("\nNeed a distributed cdeepsys?\n",
		g_version_string.c_str());

	wattrset(m_win, parent->m_colors[sinsp_cursesui::PROCESS]);
	m_ctext->printf("Deepsys cloud offers distributed cdeepsys functionality, a powerful web interface and much more.\nwww.deepsys.com.\n");
...
deepsys_table_action curses_mainhelp_page::handle_input(int ch)
{
	int32_t totlines;
