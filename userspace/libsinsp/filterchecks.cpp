...
const filtercheck_field_info sinsp_filter_check_event_fields[] =
{
	...
	{PT_CHARBUF, EPF_NONE, PF_NA, "syscall.type", "For system call events, the name of the system call (e.g. 'open'). Unset for other events (e.g. switch or deepsys internal events). Use this field instead of evt.type if you need to make sure that the filtered/printed value is actually a system call."},
	...
	case TYPE_LATENCY_QUANTIZED:
		{
			if(evt->m_tinfo != NULL)
			{
				...
				if(lat != 0)
				{
					...
					m_u64val = (uint64_t)(llatency * g_cdeepsys_screen_w / 11) + 1;
...
const filtercheck_field_info sinsp_filter_check_tracer_fields[] =
{
	...
	{PT_CHARBUF, EPF_TABLE_ONLY, PF_NA, "span.idtag", "id used by the span list cdeepsys view."},
	{PT_CHARBUF, EPF_TABLE_ONLY, PF_NA, "span.rawtime", "id used by the span list cdeepsys view."},
	{PT_CHARBUF, EPF_TABLE_ONLY, PF_NA, "span.rawparenttime", "id used by the span list cdeepsys view."},
};
	...
	case TYPE_DURATION_QUANTIZED:
		{
			...
			else
			{
				...
				if(lat != 0)
				{
					...
					m_s64val = (uint64_t)(lduration * g_cdeepsys_screen_w / 11) + 1;
...
const filtercheck_field_info sinsp_filter_check_container_fields[] =
{
	...
	{PT_CHARBUF, EPF_NONE, PF_NA, "container.image", "the container image name (e.g. deepsys/deepsys:latest for docker, )."},
