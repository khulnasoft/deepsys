...
public:
		...
		bool is_drilldown)
	{
		...
		m_prev_is_filter_deepsys = prev_is_filter_deepsys;
...
public:
	...
	deepsys_table_action handle_input(int ch);
		...
		if(m_output_type != sinsp_table::OT_JSON)
		{
			...
			if((ts - m_last_input_check_ts > m_input_check_period_ns) || m_eof)
			{
				...
				while(true)
				{
					...
					// Handle the event
					//
					deepsys_table_action ta = handle_input(input);
...
private:
	...
	bool spectro_selection(string field, string val, sinsp_view_column_info* column_info, filtercheck_field_info* info, deepsys_table_action ta);
	...
	bool execute_table_action(deepsys_table_action ta, uint32_t rownumber, bool* res);
	...
	deepsys_table_action handle_textbox_input(int ch);
