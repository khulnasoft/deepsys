	...
	m_container_manager(this)
{
	...
	m_meinfo.m_pievt.m_info = &(g_infotables.m_event_info[PPME_DEEPSYSEVENT_X]);
...
void sinsp::init()
{
	...
	{
		if(scap_getpid_global(m_h, &m_deepsys_pid) != SCAP_SUCCESS)
		{
			ASSERT(false);
		}
	}
...
void sinsp::init_mesos_client(string* api_server, bool verbose)
{
	...
	if(m_mesos_client == NULL)
	{
		...
		m_mesos_client = new mesos(m_mesos_api_server,
									...
									mesos::credentials_t(), // mesos creds, the only way to provide creds in deepsys is embedded in URI
