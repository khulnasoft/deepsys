...
USAGE: deepsys -c memcachelog <get|set>
  eg,

  deepsys -c memcachelog             # show memcached get/set utilization
  deepsys -c memcachelog get         # show memcached only get utilization
  deepsys -c memcachelog set         # show memcached only set utilization
  deepsys -c memcachelog 'set 1000'  # show memcached set utilization which object's size is higher than 1000
...
function on_init()
    util = {}
    start_time = os.time()
    deepsys.set_filter("(fd.sport=11211 or proc.name=memcached) and evt.is_io=true")
    deepsys.set_snaplen(4096)
