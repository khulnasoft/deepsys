...
-- Chisel description
description = "Given a filter on the command line, this chisel saves the events that are in a time range around filter matches, and that are on the SAME process/thread. The time range can be adjusted with the dump_range_ms argument. For example, 'deepsys -c around evt.type=open and evt.failed=true' will save two seconds of activity around every failed open.";
...
function on_init()	
	...
	is_tty = deepsys.is_tty()
	
	if deepsys.get_filter() == "" then
...
function on_capture_end()
	...
	if capturing then
		local sn = deepsys.get_evtsource_name()
